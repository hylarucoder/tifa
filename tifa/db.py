...
class Model(DeclarativeBase):
    ...
    @classmethod
    async def async_find_by_id(cls, session: AsyncSession, id: str | int) -> Self | None:
    ...
    @classmethod
    async def async_bulk_create(cls, session: AsyncSession, items: list[dict]) -> list[Self]:
        ins_list = []
        for item in items:
            ins = cls(**item)
            ins_list.append(ins)
    ...
    @classmethod
    async def async_find_first_or_404(cls, session: AsyncSession, stmt: sa.Select) -> Self:
